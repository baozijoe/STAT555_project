...
out_path="/storage/work/fjz5078/Courses/STAT555/project/2-out/ATAC-seq/CMP_HSC/"

cell1_rep1 = "/storage/work/fjz5078/Courses/STAT555/project/0-rawdata/ATAC-seq/HSC_vs_CMP/HSC1.tab"
cell1_rep2 = "/storage/work/fjz5078/Courses/STAT555/project/0-rawdata/ATAC-seq/HSC_vs_CMP/HSC2.tab"
cell2_rep1 = "/storage/work/fjz5078/Courses/STAT555/project/0-rawdata/ATAC-seq/HSC_vs_CMP/CMP1.tab"
cell2_rep2 = "/storage/work/fjz5078/Courses/STAT555/project/0-rawdata/ATAC-seq/HSC_vs_CMP/CMP2.tab"

# 读取CSV文件
RNA_data <- read.csv("/storage/work/fjz5078/Courses/STAT555/project/2-out/RNA_seq/HSC_CMP/HSC_CMP_DGE.csv", header = TRUE)
...
sample_info <- data.frame(
  cell_type = rep(c("HSC", "CMP"), each = 2),
  replicate = rep(1:2, 2),
...
# 创建图形对象
 plot <- ggplot() +
  geom_segment(data = gene_positions, aes(x = Start, xend = End, y = Chromosome, yend = Chromosome), color = "purple", size = 5) +
  geom_segment(data = significant_positions, aes(x = Start, xend = End, y = Chromosome, yend = Chromosome), color = "red", size = 5) +
  theme_minimal() +
...
ggsave(file.path(out_path,"EnhancedVolcano.png"), plot = pie_chart, width = 10, height = 8, dpi = 300)
...
# 写入CSV文件
write.csv(nearest_gene_details, file.path(out_path,"nearest_gene_details.csv"), row.names = FALSE)
...
write.csv(gene_ids, file.path(out_path,"gene_ids.csv"), row.names = FALSE)
...
ggsave(file.path(out_path,"scatter_plot_log2.png"), plot = p, width = 8, height = 6, dpi = 300)
...
ggsave(file.path(out_path, "scatter_plot_p.png"), plot = p_1, width = 8, height = 6, dpi = 300)
